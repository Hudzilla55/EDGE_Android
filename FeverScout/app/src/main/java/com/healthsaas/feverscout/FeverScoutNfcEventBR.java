...
import static com.healthsaas.feverscout.nfc.Util.getNfcDomain;
...
public class FeverScoutNfcEventBR extends BroadcastReceiver {
    private static final String TAG = FeverScoutNfcEventBR.class.getSimpleName();
    Context mAppContext = null;

    public FeverScoutNfcEventBR() {
        super();
    }
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Constants.NFC_TECH_ACTION.equals(action) ||
                    ...
                    if (ndefMsg != null) {
                        ...
                        for (NdefRecord ndefRec : ndefRecords) {
                            ...
                            if (MIME_TYPE.equals(ndefRec.toMimeType())) {
                                ...
                                if (tokens != null) {
                                    if (Constants.COM_HEALTHSAAS_FEVERSCOUT.equals(getNfcDomain(tokens))) {
                                        handleDomain(tokens);
                                    }
                                }
                            }
            ...
            //Intent i = new Intent(mAppContext, VivaLinkFeverScoutManager.class);
            //i.setAction(VivaLinkFeverScoutManager.ACTION_SCAN_EVENT);
            //i.putExtras(intent);
    ...
    private void sendZewaManagerIntent(int cmdId, String model, String bt_mac) {
        ...
        //bundle.putInt(VivaLinkFeverScoutManager.KEY_CMD_ID, cmdId);
        //bundle.putString(VivaLinkFeverScoutManager.KEY_MODEL_NAME, model);
        //bundle.putString(VivaLinkFeverScoutManager.KEY_BT_MAC, bt_mac);

        //Intent intent = new Intent(mAppContext, VivaLinkFeverScoutManager.class);
        //intent.setAction(VivaLinkFeverScoutManager.ACTION_NFC_EVENT)
        //        .putExtras(bundle);
