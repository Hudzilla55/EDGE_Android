...
//		TimeZone.setDefault(TimeZone.getTimeZone("UTC"));
	...
	public static String formatTimePercentage(long time, long count){
		NumberFormat numberFormat= NumberFormat.getInstance();
		numberFormat.setMaximumFractionDigits(0);
		String result = numberFormat.format((float) time / (float) count * 100);
		if(time - 3600 >=0){
			return  time/(60*60)+" hr "+ time%(60*60)/60+" min " +  result+"%";
		}
		else if(time >=60){
			return  " 0 hr "+ time/60+" min " +  result+"%";
		}
		else{
			return  " 0 hr "+ time+" min "+  result+"%";
		}
	}

	public static String formatTime(long time){
		if(time - 3600 >=0){
			return  time/(60*60)+" hr "+ time%(60*60)/60+" min ";
		}
		else if(time >=60){
			return  " 0 hr "+ time/60+" min ";
		}
		else{
			return  " 0 hr "+ time+" min ";
		}
	}
}
