...
import static com.healthsaas.zewamedwell.nfc.Util.getNfcDomain;
...
public class ZewaMedWellNfcEventBR extends BroadcastReceiver {
    private static final String TAG = ZewaMedWellNfcEventBR.class.getSimpleName();
    Context mAppContext = null;

    public ZewaMedWellNfcEventBR() {
        super();
    }
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Constants.NFC_TECH_ACTION.equals(action) ||
                    ...
                    if (ndefMsg != null) {
                        ...
                        for (NdefRecord ndefRec : ndefRecords) {
                            ...
                            if (MIME_TYPE.equals(ndefRec.toMimeType())) {
                                ...
                                if (tokens != null) {
                                    if (Constants.DOMAIN_HEALTHSAAS_ZEWAMEDWELL.equals(getNfcDomain(tokens))) {
                                        handleDomain(tokens);
                                    }
                                }
                            }
            ...
            Intent i = new Intent(mAppContext, ZewaMedWellManager.class);
            i.setAction(ZewaMedWellManager.ACTION_SCAN_EVENT);
            i.putExtras(intent);
    ...
    private void sendZewaManagerIntent(int cmdId, String model, String bt_mac) {
        ...
        bundle.putInt(ZewaMedWellManager.KEY_CMD_ID, cmdId);
        bundle.putString(ZewaMedWellManager.KEY_MODEL_NAME, model);
        bundle.putString(ZewaMedWellManager.KEY_BT_MAC, bt_mac);

        Intent intent = new Intent(mAppContext, ZewaMedWellManager.class);
